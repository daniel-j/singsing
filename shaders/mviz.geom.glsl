...
float sample_freq(float x) {
    return max(-1., .1*log(8.*texture(iFreq, .5*pow(mix(x, 1., .13), 3.)).r) - .4);
    //return max(-1., sqrt(1.2*texture(iFreq, pow(mix(x, 1., .18), 4.)).r) - 1.);
}
...
float smooth_wave(float x) {
    ...
    const float width = 7.;
    for (float i = - width; i <= width; i += 1.)
        sum += .55 * texture(iSound, x + i / iNumGeomIters).r + .5;
    return sum / (2. * width + 1.);
}
