...
bool Program::load(const std::string &vertexShaderPath, const std::string &fragmentShaderPath/*, const std::string &geometryShaderPath = ""*/) {
...
bool Program::LoadShader(const std::string &fileName, GLenum shaderType) {
...
void Program::setUniform(const std::string& name, GLfloat v0) {
    GLCall(glUniform1f(uniform(name), v0));
}
void Program::setUniform(const std::string& name, GLfloat v0, GLfloat v1) {
    GLCall(glUniform2f(uniform(name), v0, v1));
}
void Program::setUniform(const std::string& name, GLfloat v0, GLfloat v1, GLfloat v2) {
    GLCall(glUniform3f(uniform(name), v0, v1, v2));
}
void Program::setUniform(const std::string& name, GLfloat v0, GLfloat v1, GLfloat v2, GLfloat v3) {
    GLCall(glUniform4f(uniform(name), v0, v1, v2, v3));
}

void Program::setUniformi(const std::string& name, GLint v0) {
    GLCall(glUniform1i(uniform(name), v0));
}
void Program::setUniformi(const std::string& name, GLint v0, GLint v1) {
    GLCall(glUniform2i(uniform(name), v0, v1));
}

void Program::setUniformui(const std::string& name, GLuint v0) {
    GLCall(glUniform1ui(uniform(name), v0));
}
void Program::setUniformui(const std::string& name, GLuint v0, GLuint v1) {
    GLCall(glUniform2ui(uniform(name), v0, v1));
}

// Tries to compile the shader. Returns false if something fails
bool Program::TryCompileShader(int shaderId, const std::string &fileName) {
...
int Program::CreateShader(const std::string &fileName, GLenum shaderType) {
...
std::string Program::ReadFile(const char* file) {
    // Open file
    std::ifstream t(file);
...
void Program::PrintShaderLinkingError(int32_t shaderId) {
    std::cerr << "=======================================\n";
...
void Program::PrintShaderCompilationErrorInfo(int32_t shaderId, const std::string &fileName) {
    // Find length of shader info log
    int maxLength;
